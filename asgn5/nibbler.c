...
*             mode     - The mode string used for fopen()
*
* Return: a pointer to the NIB, also return NULL on failure.

* write header comment for more details.
*/
...
void nib_put_nibble(int nibble, NIB *nib) {
    ...
    nib->stored_nibble = nibble & 0x0F; //make sure only 4 bits are stored
    nib->num_nibbles = 1;
    }
}
...
*
*
* Parameters: nib - the NIB to close
*
*
...
void nib_close(NIB *nib) {
    ...
    if(nib->num_nibbles > 0 && !nib->opened_for_read){
